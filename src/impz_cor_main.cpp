...
int main(int argc, char **argv) {
	...
	// load typed snps
	vector<typed_snp> typed_snps_tmp;
	size_t num_typed_snps = load_typed_snps(IN_TYPED_SNP_FILE, typed_snps_tmp);
    ...
    intersect_snps(typed_snps_tmp, all_snps, convert_flags, impute_flags, typed_idx_map);
	...
	for(size_t i = 0; i < num_typed_snps; i++) {
		if(convert_flags[i] == -1) {
			typed_snps_tmp[i].zscore *= -1.0;
		}
	}

    // get typed snps used for imputation
    vector<typed_snp> typed_snps;
    for(size_t i=0; i<num_typed_snps; i++) {
        if(convert_flags[i] != 0) {
            typed_snps.push_back(typed_snps_tmp[i]);
        }
    }
    num_typed_snps = typed_snps.size();
