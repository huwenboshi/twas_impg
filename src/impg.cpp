...
size_t load_cors(const char *filename, vector<double> &cors) {
	...
	// read in the rest
	double cor;
	while(fscanf(fin, "%lf", &cor)>0) {
		cors.push_back(cor);
	}
	fclose(fin);
	
	size_t num_cors = cors.size();
	if(verbose) {
		printf("Info: Loaded %u cors from %s...\n",
			(unsigned int)num_cors, filename);
	}
		...
		if(impute_flags[idx] == 1) {
			...
			for(size_t i = 0; i < num_typed_snps; i++) {
				if(filter_flags[i] != 1) {
					...
					double r = corr(haps[idx], haps[sidx]);
					sigma_it[ii] = r;
					ii++;
				}
			}
...
void get_beta_var_cor(const string& prefix, const vector<ref_snp>& all_snps,
    ...
    const vector<char>& filter_flags, const vector<int>& typed_idx_map,
    double *sigma_t_tmp, double lambda, const vector<double> &cors,
    size_t sigma_dim) {
	...
	// count the number of snps used for imputation
	size_t num_snps = 0;
	for(size_t i = 0; i < num_typed_snps; i++) {
		if(filter_flags[i] != 0) {
			num_snps++;
	...
	for(size_t i = 0; i < num_typed_snps; i++) {
		for(size_t j = 0; j < num_typed_snps; j++) {
			if(filter_flags[i] != 0 && filter_flags[j] != 0) {
                size_t ld_i = (size_t) typed_idx_map[i];
				size_t ld_j = (size_t) typed_idx_map[j];
                sigma_t[ii] = sigma_t_tmp[ld_i*sigma_dim+ld_j];
				if(ld_i == ld_j) {
					sigma_t[ii] += lambda;
				}
				ii++;
	...
	for(size_t idx = 0; idx < num_total_snps; idx++) {
		...
		if(impute_flags[idx] == 1) {
			...
			for(size_t i = 0; i < num_typed_snps; i++) {
				if(filter_flags[i] != 0) {
					sigma_it[ii] = cors[typed_idx_map[i]];
					ii++;
				}
			}
		}
...
// get the command line input for gen_beta program
int get_gen_beta_cmd_line_cor(int argc, char **argv, char **IN_LD_FILE,
		char **IN_ALL_SNP_FILE, char **IN_TYPED_SNP_FILE,
		char **OUT_FILE_PREFIX, char **IN_COR_FILE) {
	int nflags = 0;
	char opt;
	while((opt = getopt(argc,argv,"l:m:t:p:c:")) != -1) {
		switch(opt) {
			case 'l': // ld file
			...
			case 'p': // prefix for output files
				*OUT_FILE_PREFIX = (char *) strdup(optarg);
				nflags++;
				break;
            case 'c': // input cor file
                *IN_COR_FILE = (char *) strdup(optarg);
                nflags++;
                break;
		}
	}
	if(!(*IN_LD_FILE) || !(*IN_ALL_SNP_FILE) || !(*IN_TYPED_SNP_FILE) ||
       !(*OUT_FILE_PREFIX) || !(*IN_COR_FILE)) {
		fprintf(stderr, "Usage:\n");
        fprintf(stderr, "\t-l (required) specify LD file\n");
		fprintf(stderr, "\t-m (required) specify SNP mapping file\n");
		fprintf(stderr, "\t-t (required) specify typed SNP file\n");
		fprintf(stderr, "\t-p (required) specify output file prefix\n");
        fprintf(stderr, "\t-c (required) specify user defined cor file\n");
		exit(1);
	}
